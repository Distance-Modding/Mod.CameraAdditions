...
namespace Distance.CameraAdditions
{
	...
	public class Mod : MonoBehaviour
	{
		...
		public void OnConfigChanged(ConfigLogic config)
		{
			...
			BindAction(ref _keybindZoomIn, config.ZoomInHotkey, () => Config.ZoomOffset += 0.5f);

			BindAction(ref _keybindZoomOut, config.ZoomOutHotkey, () => Config.ZoomOffset -= 0.5f);

			BindAction(ref _keybindDefaults, config.DefaultsHotkey, () => SetDefaults());

			BindAction(ref _keybindIncreaseXOffset, config.IncreaseXOffsetHotkey, () => Config.XOffset += 0.5f);

			BindAction(ref _keybindDecreaseXOffset, config.DecreaseXOffsetHotkey, () => Config.XOffset -= 0.5f);

			BindAction(ref _keybindIncreaseYOffset, config.IncreaseYOffsetHotkey, () => Config.YOffset += 0.5f);

			BindAction(ref _keybindDecreaseYOffset, config.DecreaseYOffsetHotkey, () => Config.YOffset -= 0.5f);
		}
